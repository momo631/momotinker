...
public class OverCowardiceSin extends momomodifier {
    ...
    public float getMeleeDamage(@Nonnull IToolStackView tool, ModifierEntry modifier, @Nonnull ToolAttackContext context, float baseDamage, float damage) {
        ...
        int a = getAllModifierlevel(attacker,MomotinkerModifiers.overcowardicesin.getId());
    ...
    public void addTooltip(IToolStackView tool, ModifierEntry modifierEntry, @org.jetbrains.annotations.Nullable Player player, List<Component> tooltip, TooltipKey tooltipKey, TooltipFlag tooltipFlag) {
        if (player != null) {
            int a = getAllModifierlevel(player,MomotinkerModifiers.overcowardicesin.getId());
            if (a>0&&a<8) {
