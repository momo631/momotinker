    ...
    public static void handlePacket(KeyAInputPKT pkt, Supplier<NetworkEvent.Context> context$) {
        ...
        context.enqueueWork(() -> {
            ...
            if (player!=null&&player.getMainHandItem().is(entropy_burning_cube.get())){
                ...
                ItemStack itemStack = ToolStack.createTool(entropy_burning_sword.get(), MomoToolDefinitions.ENTROPY_BURNING_SWORD,tool.getMaterials()).createStack();
            ...
            if (player!=null&&player.getMainHandItem().is(entropy_burning_sword.get())){
                ItemStack stack = player.getMainHandItem();
                ToolStack tool = ToolStack.from(stack);
                ItemStack itemStack = ToolStack.createTool(entropy_burning_riding_spear.get(), MomoToolDefinitions.ENTROPY_BURNING_RIDING_SPEAR,tool.getMaterials()).createStack();
                ToolStack tools = ToolStack.from(itemStack);
                itemStack.setTag(stack.getTag());
                tools.setUpgrades(tool.getUpgrades());
                tools.getPersistentData().copyFrom(tool.getPersistentData().getCopy());
            ...
            if (player!=null&&player.getMainHandItem().is(entropy_burning_riding_spear.get())){
                ItemStack stack = player.getMainHandItem();
                ToolStack tool = ToolStack.from(stack);
                ItemStack itemStack = ToolStack.createTool(entropy_burning_cannon.get(), MomoToolDefinitions.ENTROPY_BURNING_CANNON,tool.getMaterials()).createStack();
                ToolStack tools = ToolStack.from(itemStack);
                itemStack.setTag(stack.getTag());
                tools.setUpgrades(tool.getUpgrades());
                tools.getPersistentData().copyFrom(tool.getPersistentData().getCopy());
            ...
            if (player!=null&&player.getMainHandItem().is(entropy_burning_cannon.get())){
                ItemStack stack = player.getMainHandItem();
                ToolStack tool = ToolStack.from(stack);
                ItemStack itemStack = ToolStack.createTool(entropy_burning_cube.get(), MomoToolDefinitions.ENTROPY_BURNING_CUBE,tool.getMaterials()).createStack();
                ToolStack tools = ToolStack.from(itemStack);
                itemStack.setTag(stack.getTag());
                tools.setUpgrades(tool.getUpgrades());
                tools.getPersistentData().copyFrom(tool.getPersistentData().getCopy());
