...
import com.momosensei.momotinker.entity.PullBEntity;
...
public class PullBEntityRenderer extends EntityRenderer<PullBEntity> {
	private final ItemRenderer itemRenderer;
	private static final ResourceLocation CHAIN_TEXTURE = new ResourceLocation(MOD_ID,"textures/entity/pull_chain.png");
	private static final RenderType CHAIN_LAYER = RenderType.entitySmoothCutout(CHAIN_TEXTURE);
	
	public PullBEntityRenderer(EntityRendererProvider.Context renderManagerIn) {
		super(renderManagerIn);
        this.itemRenderer = renderManagerIn.getItemRenderer();
    }

	@Override
	public void render(PullBEntity entity, float yaw, float tickDelta, PoseStack matrices, MultiBufferSource provider, int light) {
		matrices.pushPose();
		...
		this.itemRenderer.renderStatic(new ItemStack(MomotinkerItem.pull_entity.get()), ItemDisplayContext.GROUND, light, OverlayTexture.NO_OVERLAY, matrices, provider, entity.level(),entity.getId());
		...
		if(fromEntity != null) {
            ...
            renderChainCube(distVec, tickDelta, entity.tickCount, matrices, provider, light);
		}
		matrices.popPose();
	}
	...
	private Vec3 getPositionOfPriorMob(Entity mob, float partialTicks){
		...
		if(mob instanceof Player){
			...
			int i = player.getMainArm() == HumanoidArm.LEFT ? 1 : -1;
	...
	@Override
	public ResourceLocation getTextureLocation(PullBEntity entity) {
