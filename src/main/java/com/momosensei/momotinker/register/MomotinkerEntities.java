...
public class MomotinkerEntities {
    ...
    public static final RegistryObject<EntityType<PullEntity>> pull_entity = ENTITIES.register("pull_entity", () -> EntityType.Builder.<PullEntity>of(PullEntity::new, MobCategory.MISC).sized(0.75F, 0.75F).setTrackingRange(4).setUpdateInterval(1).setCustomClientFactory((spawnEntity, world) -> new PullEntity(world,spawnEntity.getPosX(),spawnEntity.getPosY(),spawnEntity.getPosZ(),spawnEntity.getEntity().getDeltaMovement())).setShouldReceiveVelocityUpdates(true));
