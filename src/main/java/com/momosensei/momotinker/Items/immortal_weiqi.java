...
public class immortal_weiqi extends Item {
    public immortal_weiqi(Properties properties) {
        super(properties);
    }
    @Override
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, List<Component> list, @NotNull TooltipFlag flag) {
        ...
        if (Screen.hasShiftDown()&&config&&configa) {
            ...
            list.add(Component.translatable("item.momotinker.tooltip.immortal_weiqi1").withStyle(ChatFormatting.WHITE));
            if (config&&configa) {
