    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, List<Component> list, @NotNull TooltipFlag flag) {
        ...
        if (Screen.hasShiftDown() && config && configa) {
            if (StageMeteor.getStageFloat() == 1 || !configb) {
                list.add(Component.translatable("item.momotinker.tooltip.heartsteel2").withStyle(ChatFormatting.DARK_GREEN));
            } else {
                list.add(Component.translatable("item.momotinker.tooltip.stage_meteor").withStyle(ChatFormatting.GOLD));
            }
        } else {
            list.add(net.minecraft.network.chat.Component.translatable("item.momotinker.tooltip.heartsteel1").withStyle(ChatFormatting.DARK_GREEN));
            if (config && configa) {
